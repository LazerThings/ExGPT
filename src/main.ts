...
const TOGGLES: Toggle[] = [
  ...
  },
  {
    ...
    icon: 'ph-eyeglasses',
    prompt: 'You can output live HTML previews using a special code block. Use ```live followed by a COMPLETE, properly formatted HTML document starting with <!DOCTYPE html> and including <html>, <head>, and <body> tags. The HTML will be rendered in a live preview frame. The document must be fully self-contained and valid. IMPORTANT: Please only use Live HTML when you think it is useful for the situation or the user asks you to use it. Do not use it in every case. Use it ONLY if it would provide important additional visual aids or if interactivity is important - not just because putting it in cards would be nice. If normal text/markdown would work fine for the usecase, please do not use Live HTML. DO NOT make "visual showcases" just to be able to use Live HTML unless it is ABSOLUTELY NECCESARY. If you could type it in text and you don\'t need HTML for interactivity or display, don\'t use HTML.',
    dependsOn: 'markdown',
  },
  {
